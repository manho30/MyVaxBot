...
const getCases = (dates) => {
    const req = UrlFetchApp.fetch("https://raw.githubusercontent.com/MoH-Malaysia/covid19-public/main/epidemic/cases_malaysia.csv").toString();
    const res = req.split(/\n/);
    return res
        .filter((res) => {
            const data = res.split(",");
...
const getSchoolVaccination = (code) => {
    const req = UrlFetchApp.fetch("https://raw.githubusercontent.com/MoH-Malaysia/covid19-public/main/vaccination/vax_school.csv").toString()
    const res = req.split(/\n/);
    return res
        .filter((res) => {
            const data = res.split(",");

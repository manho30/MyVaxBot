const getFormatCase = (dates) => {
    if(!dates){
        dates = getDates()
    }
    const data = getCases(dates)
...
const getFormatSchoolVaccination = (code) => {
    const data = getSchoolVaccination(code);
...
const getFormatVaccination = (dates) => {
    if (!dates) {
        dates = getDates()
    }
    const data = getVaccination(dates)
    if(!isNaN(parseInt(data[0].total_dose_2))){
        const results = (data) => {
            const  fully_percent = "*Fully Vaccinated* (" + vaccinatedPercentage(parseInt(data[0].total_dose_2), population) + ")\n" + generateBar(parseInt(data[0].total_dose_2)) + "\n\n"
            const  booster_percent = "*Booster Dose* (" + vaccinatedPercentage(parseInt(data[0].total_booster_dose), population) + ")\n" + generateBar(parseInt(data[0].total_booster_dose)) + "\n\n\n"
            ...
            const res = fully_percent + booster_percent + fully_vaccinated + first_dose + booster_dose + kid_first_dose + kid_fully_vaccinated + total_dose_given + "\n\nData as " + data[0].date
            return res
        }
        return results(data)
    } else {
